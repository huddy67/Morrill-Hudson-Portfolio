...
In this part of the project, we learned how to troubleshoot through network issues on both Mac and Ubuntu. Some things we learned how to do are listed below.

- How to check IP Address
- How to test connection
- Howe to find and make sure our default gateway works
- Test DNS

I thought I learned a lot in this project, especially about doing something simple like checking our ip address. However, I also dealt with a lot of issues, mainly in the later part of it in step 4 B. When I ran the command "ping -c 4 8.8.8.8", I got an error saying command not found. I checked the speeling and spaces to make sure everything was correct, and it was. I then ran a few commands, "sudo ufw disable", and then "sudo unminimize". I thought this might fix the issue, but when I did "ping -c 4 8.8.8.8", I still got the same error. I tried to discuss this with both my classmates and teachers, but we could not find a solution. We checked to make sure that I was on enf1, since there were issues with other computers not being able to run the commands on enf0. Everything appeared to be correct, so I was unable to complete the rest of the project. I documented this on my asignment that I turned in. The issue only happened with one other computer in the classroom, so it could have been an issue with the switch over from ethernet to wifi. Nontheless, I still think I learned a lot from the previous sections of the proejct as well as from troubleshooting my issues in this section.

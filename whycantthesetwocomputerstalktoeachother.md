...
Even though the two computers were connected through a simple Ethernet cable, they were able to communicate successfully because UTM placed both virtual machines on the same Layer 2 broadcast domain and the same Layer 3 network. Since they shared the same virtual switch, ARP requests and MAC address discovery worked correctly, which allowed each VM to learn the other’s hardware address. With matching subnets and a shared virtual adapter, the packets moved across the network without any issues. This setup functioned much like a small home network, where devices on the same switch can immediately exchange traffic as long as their IP information is configured properly. Because the VMs were aligned on both Layer 2 and Layer 3, the network connection behaved exactly as expected, and the communication worked smoothly. This was not supposed to happen because they were supposed not to be able to communicate with us.
